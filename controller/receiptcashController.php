<?php
Class receiptcashController Extends baseController {
    public function index() {
        ...
        $data = array(
            'where' => 'payment_bank_type=1 AND payment_type=1 AND payment_check=1 AND payment_in_ex=1',
        );
        ...
        $this->view->data['lastID'] = isset($payment_model->getPaymentByWhere(array('payment_bank_type'=>1,'payment_type'=>1,'payment_check'=>1,'payment_in_ex'=>1))->payment_document_number)?$payment_model->getPaymentByWhere(array('payment_bank_type'=>1,'payment_type'=>1,'payment_check'=>1,'payment_in_ex'=>1))->payment_document_number:0;

        /* Lấy tổng doanh thu*/
        
        /*************/
        $this->view->show('receiptcash/index');
    }
    public function printview() {
        ...
        $this->view->show('receiptcash/printview');
    }
   ...
   public function getItem(){
        ...
        $qr = 'SELECT a.*, d.customer_name, d.customer_code, d.customer_id FROM invoice_sell a, customer d WHERE a.invoice_sell_customer=d.customer_id AND  (SELECT SUM(c.debit_money) FROM debit c WHERE c.invoice_sell=a.invoice_sell_id GROUP BY c.invoice_sell)>0 ';

        $debits = $debit_model->queryDebit($qr);
        
        $str = '<table class="table_data" id="tblExport2">';
        $str .= '<thead><tr><th class="fix"><input type="checkbox" onclick="checkall(\'checkbox2\', this)" name="checkall"/></th><th class="fix">Số CT</th><th class="fix">Ngày CT</th><th class="fix">Khách hàng</th><th class="fix">Diễn giải</th><th class="fix">Số tiền phải thu</th><th class="fix">Số tiền đã trả</th><th class="fix">Còn lại</th></tr></thead>';
        $str .= '<tbody>';

        foreach ($debits as $debit) {
            $pays = $debit_model->getAllDebit(array('where'=>'debit_money<0 AND invoice_sell='.$debit->invoice_sell_id));
            $used=0;
            foreach ($pays as $pay) {
                $used += $pay->debit_money;
            }
            $money = $debit->invoice_sell_total+$used;
            $str .= '<tr class="tr" data="'.$debit->invoice_sell_id.'"><td><input name="check_i[]" type="checkbox" class="checkbox2" data="'.$rowIndex.'" value="'.$debit->invoice_sell_id.'" data-doc="'.$debit->invoice_sell_document_number.'" data-date="'.date('d/m/Y',$debit->invoice_sell_document_date).'" data-invoice="'.$debit->invoice_sell_number.'" data-cus="'.$debit->invoice_sell_customer.'" data-cuscode="'.$debit->customer_code.'" data-cusname="'.$debit->customer_name.'" data-rece="'.$this->lib->formatMoney($debit->invoice_sell_total).'" data-pay="'.$this->lib->formatMoney($used).'" data-money="'.$this->lib->formatMoney($money).'" ></td><td class="fix">'.$debit->invoice_sell_document_number.'</td><td class="fix">'.$this->lib->hien_thi_ngay_thang($debit->invoice_sell_document_date).'</td><td class="fix">'.$debit->customer_name.'</td><td class="fix">'.$debit->invoice_sell_comment.'</td><td class="fix">'.$this->lib->formatMoney($debit->invoice_sell_total).'</td><td class="fix">'.$this->lib->formatMoney($used).'</td><td class="fix"><input style="width:120px" type="text" name="money_add[]" class="money_add numbers" value="'.$this->lib->formatMoney($money).'" max="'.$money.'"></td></tr>';
        }
        ...
        if (isset($_POST['payment'])) {
            ...
            foreach ($payment_items as $item) {
                ...
                if ($item->payment_item_invoice > 0) {
                    $invoices = $invoice_sell_model->getInvoice($item->payment_item_invoice);
                    $invoice_id = $invoices->invoice_sell_id;
                    $invoice_number = $invoices->invoice_sell_document_number;
                    $invoice_date =  $invoices->invoice_sell_document_date;
                    $invoice = $invoices->invoice_sell_number;
                }
