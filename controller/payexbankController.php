<?php
Class payexbankController Extends baseController {
    public function index() {
        ...
        $data = array(
            'where' => 'payment_bank_type=2 AND payment_type=2 AND payment_check=1 AND payment_in_ex=2',
        );
        ...
        $this->view->data['lastID'] = isset($payment_model->getPaymentByWhere(array('payment_bank_type'=>2,'payment_type'=>2,'payment_check'=>1,'payment_in_ex'=>2))->payment_document_number)?$payment_model->getPaymentByWhere(array('payment_bank_type'=>2,'payment_type'=>2,'payment_check'=>1,'payment_in_ex'=>2))->payment_document_number:0;

        /* Lấy tổng doanh thu*/
        
        /*************/
        $this->view->show('payexbank/index');
    }
   ...
   public function getItem(){
        ...
        $qr = 'SELECT a.*, d.customer_name, d.customer_code, d.customer_id FROM invoice_buy a, customer d WHERE a.invoice_buy_customer=d.customer_id AND  (SELECT SUM(c.debit_money) FROM debit c WHERE c.invoice_buy=a.invoice_buy_id GROUP BY c.invoice_buy)>0 ';

        $debits = $debit_model->queryDebit($qr);
        
        $str = '<table class="table_data" id="tblExport2">';
        $str .= '<thead><tr><th class="fix"><input type="checkbox" onclick="checkall(\'checkbox2\', this)" name="checkall"/></th><th class="fix">Số CT</th><th class="fix">Ngày CT</th><th class="fix">Khách hàng</th><th class="fix">Diễn giải</th><th class="fix">Số tiền NT</th><th class="fix">Số tiền NT đã trả</th><th class="fix">Số tiền phải trả</th><th class="fix">Số tiền đã trả</th><th class="fix">Còn lại</th></tr></thead>';
        $str .= '<tbody>';

        foreach ($debits as $debit) {
            $pays = $debit_model->getAllDebit(array('where'=>'debit_money<0 AND invoice_buy='.$debit->invoice_buy_id));
            $used=0;
            $used_foreign=0;
            foreach ($pays as $pay) {
                $used += $pay->debit_money;
                $used_foreign += round($pay->debit_money_foreign,2);
            }
            $money = $debit->invoice_buy_money+$used;
            $money_foreign = round($debit->invoice_buy_money_foreign+$used_foreign,2);
            $str .= '<tr class="tr" data="'.$debit->invoice_buy_id.'"><td><input name="check_i[]" type="checkbox" class="checkbox2" data-invoice-type="1" data="'.$rowIndex.'" value="'.$debit->invoice_buy_id.'" data-doc="'.$debit->invoice_buy_document_number.'" data-date="'.date('d/m/Y',$debit->invoice_buy_document_date).'" data-invoice="'.$debit->invoice_buy_number.'" data-cus="'.$debit->invoice_buy_customer.'" data-cuscode="'.$debit->customer_code.'" data-cusname="'.$debit->customer_name.'" data-rece="'.$this->lib->formatMoney($debit->invoice_buy_money).'" data-pay="'.$this->lib->formatMoney($used).'" data-money="'.$this->lib->formatMoney($money).'" data-rece-foreign="'.$this->lib->formatMoney($debit->invoice_buy_money_foreign,true).'" data-pay-foreign="'.$this->lib->formatMoney($used_foreign,true).'" data-money-foreign="'.$this->lib->formatMoney($money_foreign,true).'" ></td><td class="fix">'.$debit->invoice_buy_document_number.'</td><td class="fix">'.$this->lib->hien_thi_ngay_thang($debit->invoice_buy_document_date).'</td><td class="fix">'.$debit->customer_name.'</td><td class="fix">'.$debit->invoice_buy_comment.'</td><td class="fix">'.$this->lib->formatMoney($debit->invoice_buy_money_foreign,true).'</td><td class="fix">'.$this->lib->formatMoney($used_foreign,true).'</td><td class="fix">'.$this->lib->formatMoney($debit->invoice_buy_money).'</td><td class="fix">'.$this->lib->formatMoney($used).'</td><td class="fix"><input style="width:120px" type="text" name="money_add[]" class="money_add numbers" value="'.$this->lib->formatMoney($money).'" max="'.$money.'"></td></tr>';
        }

        $qr = 'SELECT a.*, d.customer_name, d.customer_code, d.customer_id FROM service_buy a, customer d WHERE a.service_buy_customer=d.customer_id AND  (SELECT SUM(c.debit_money_foreign) FROM debit c WHERE c.service_buy=a.service_buy_id GROUP BY c.service_buy)>0 ';
        $debits = $debit_model->queryDebit($qr);
        ...
        foreach ($debits as $debit) {
            ...
            $money_foreign = round($debit->service_buy_money_foreign+$used_foreign,2);
            $str .= '<tr class="tr" data="'.$debit->service_buy_id.'"><td><input name="check_i[]" type="checkbox" class="checkbox2" data-invoice-type="2" data="'.$rowIndex.'" value="'.$debit->service_buy_id.'" data-doc="'.$debit->service_buy_document_number.'" data-date="'.date('d/m/Y',$debit->service_buy_document_date).'" data-invoice="'.$debit->service_buy_number.'" data-cus="'.$debit->service_buy_customer.'" data-cuscode="'.$debit->customer_code.'" data-cusname="'.$debit->customer_name.'" data-rece="'.$this->lib->formatMoney($debit->service_buy_total).'" data-pay="'.$this->lib->formatMoney($used).'" data-money="'.$this->lib->formatMoney($money).'" data-rece-foreign="'.$this->lib->formatMoney($debit->service_buy_money_foreign,true).'" data-pay-foreign="'.$this->lib->formatMoney($used_foreign,true).'" data-money-foreign="'.$this->lib->formatMoney($money_foreign,true).'"  ></td><td class="fix">'.$debit->service_buy_document_number.'</td><td class="fix">'.$this->lib->hien_thi_ngay_thang($debit->service_buy_document_date).'</td><td class="fix">'.$debit->customer_name.'</td><td class="fix">'.$debit->service_buy_comment.'</td><td class="fix">'.$this->lib->formatMoney($debit->service_buy_money_foreign,true).'</td><td class="fix">'.$this->lib->formatMoney($used_foreign,true).'</td><td class="fix">'.$this->lib->formatMoney($debit->service_buy_total).'</td><td class="fix">'.$this->lib->formatMoney($used).'</td><td class="fix"><input style="width:120px" type="text" name="money_add[]" class="money_add numbers" value="'.$this->lib->formatMoney($money).'" max="'.$money.'"></td></tr>';
        }
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $data = array(
                        ...
                        'payment_money_type' => trim($_POST['payment_money_type']),
                        'payment_money_rate' => str_replace(',', '', $_POST['payment_money_rate']),
                        );
            ...
            if (isset($id_payment)) {
                ...
                foreach ($items as $v) {
                    if($v['payment_item_customer'] != ""){
                        ...
                        $data_item = array(
                            ...
                            'payment_item_receivable_money_foreign' => str_replace(',', '', $v['payment_item_receivable_money_foreign']),
                            'payment_item_pay_money_foreign' => str_replace(',', '', $v['payment_item_pay_money_foreign']),
                            'payment_item_price' => str_replace(',', '', $v['payment_item_price']),
                        ...
                        $money_foreign += $data_item['payment_item_price'];
                        ...
                        if ($payment_items) {
                            ...
                            $data_debit = array(
                                ...
                                'debit_money_foreign'=>(0-$data_item['payment_item_price']),
                ...
                $data_pay = array(
                    ...
                    'payment_money_foreign'=>$money_foreign,
                );
        ...
        if (isset($_POST['payment'])) {
            ...
            foreach ($payment_items as $item) {
                ...
                if ($item->payment_item_invoice_2 > 0) {
                    $invoices = $service_buy_model->getService($item->payment_item_invoice_2);
                    $invoice_id_2 = $invoices->service_buy_id;
                ...
                $str .= '<td class="width-5"><input value="'.$invoice.'" disabled type="text" name="payment_item_invoice_number[]" class="payment_item_invoice_number" required="required" autocomplete="off"></td>';
                ...
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_receivable_money_foreign).'" disabled type="text" name="payment_item_receivable_money_foreign[]" class="payment_item_receivable_money_foreign text-right" autocomplete="off"></td>';
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_pay_money_foreign).'" disabled type="text" name="payment_item_pay_money_foreign[]" class="payment_item_pay_money_foreign text-right" autocomplete="off"></td>';
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_receivable_money).'" disabled type="text" name="payment_item_receivable_money[]" class="payment_item_receivable_money numbers text-right" required="required" autocomplete="off"></td>';
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_pay_money).'" disabled type="text" name="payment_item_pay_money[]" class="payment_item_pay_money numbers text-right" required="required" autocomplete="off"></td>';
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_price).'" type="text" name="payment_item_price[]" class="payment_item_price numbers text-right" required="required" autocomplete="off"></td>';
                $str .= '<td class="width-7"><input value="'.$this->lib->formatMoney($item->payment_item_money).'" type="text" name="payment_item_money[]" class="payment_item_money numbers text-right" required="required" autocomplete="off"></td>';
