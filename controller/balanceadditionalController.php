<?php
Class balanceadditionalController Extends baseController {
    public function index() {
        $this->view->setLayout('admin');
        if (!isset($_SESSION['userid_logined'])) {
            return $this->view->redirect('user/login');
        }
        if (!isset(json_decode($_SESSION['user_permission_action'])->balanceadditional) || json_decode($_SESSION['user_permission_action'])->balanceadditional != "balanceadditional") {
            $this->view->data['disable_control'] = 1;
        }
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Bảng cân đối phát sinh';
        ...
        /*************/
        $this->view->show('balanceadditional/index');
    }
