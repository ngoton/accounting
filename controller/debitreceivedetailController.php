<?php
Class debitreceivedetailController Extends baseController {
    public function index() {
        $this->view->setLayout('admin');
        if (!isset($_SESSION['userid_logined'])) {
            return $this->view->redirect('user/login');
        }
        if (!isset(json_decode($_SESSION['user_permission_action'])->debitreceivedetail) || json_decode($_SESSION['user_permission_action'])->debitreceivedetail != "debitreceivedetail") {
            $this->view->data['disable_control'] = 1;
        }
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Chi tiết công nợ phải thu';
        ...
        $customer_lists = $customer_model->getAllCustomer(array('where'=>'type_customer=1','order_by'=>'customer_code ASC'));
        ...
        $invoice_sells = $invoice_sell_model->getAllInvoice(array('where'=>'invoice_sell_document_date>='.strtotime($batdau).' AND invoice_sell_document_date<'.strtotime($ngayketthuc)));
        foreach ($invoice_sells as $invoice_sell) {
            $data_invoice_sell[$invoice_sell->invoice_sell_id]['number'] = $invoice_sell->invoice_sell_document_number;
            $data_invoice_sell[$invoice_sell->invoice_sell_id]['date'] = $invoice_sell->invoice_sell_document_date;
            $data_invoice_sell[$invoice_sell->invoice_sell_id]['comment'] = $invoice_sell->invoice_sell_comment;
        }
        ...
        /*************/
        $this->view->show('debitreceivedetail/index');
    }
