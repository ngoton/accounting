<?php
Class balanceaccountingController Extends baseController {
    public function index() {
        $this->view->setLayout('admin');
        if (!isset($_SESSION['userid_logined'])) {
            return $this->view->redirect('user/login');
        }
        if (!isset(json_decode($_SESSION['user_permission_action'])->balanceaccounting) || json_decode($_SESSION['user_permission_action'])->balanceaccounting != "balanceaccounting") {
            $this->view->data['disable_control'] = 1;
        }
        $this->view->data['lib'] = $this->lib;
        $this->view->data['title'] = 'Bảng cân đối kế toán';
        ...
        /*************/
        $this->view->show('balanceaccounting/index');
    }
