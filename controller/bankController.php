...
Class bankController Extends baseController {
    ...
    public function add(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $acc_1121 = $account_model->getAccountByWhere(array('account_number'=>'1121'))->account_id;
            $bank_model = $this->model->get('bankModel');
            $data = array(
                        ...
                        'bank_debit_dauky' => str_replace(',', '', $_POST['bank_debit_dauky']),
                        );
            ...
            if ($_POST['yes'] != "") {
                ...
                if($check){
                    ...
                    $qr = $additional_model->getAdditionalByWhere(array('bank'=>$id_bank,'debit'=>$acc_1121,'additional_date'=>1));
                    if ($qr) {
                        $data_bank = array(
                            'bank'=>$id_bank,
                            'bank_balance_date'=>1,
                            'bank'=>$id_bank,
                            'bank_balance_money'=>$data['bank_debit_dauky'],
                        );
                        $bank_balance_model->updateBank($data_bank,array('bank'=>$id_bank,'bank_balance_date'=>1));
                        ...
                        $data_additional = array(
                            ...
                            'debit'=>$acc_1121,
                            'credit'=>0,
                            'money'=>$data['bank_debit_dauky'],
                        );
                        $additional_model->updateAdditional($data_additional,array('bank'=>$id_bank,'debit'=>$acc_1121,'additional_date'=>1));
                        
                    }
                    else{
                        if ($data['bank_debit_dauky']>0) {
                            $data_bank = array(
                                'bank'=>$id_bank,
                                'bank_balance_date'=>1,
                                'bank'=>$id_bank,
                ...
                $id_bank = $bank_model->getLastBank()->bank_id;
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['xoa'])) {
                ...
                foreach ($data as $data) {
                    $additional_model->queryAdditional('DELETE FROM additional WHERE bank='.$data);
                    $bank_balance_model->queryBank('DELETE FROM bank_balance WHERE bank='.$data);
                ...
                return true;
                    ...
                    $additional_model->queryAdditional('DELETE FROM additional WHERE bank='.$_POST['data']);
                    $bank_balance_model->queryBank('DELETE FROM bank_balance WHERE bank='.$_POST['data']);
